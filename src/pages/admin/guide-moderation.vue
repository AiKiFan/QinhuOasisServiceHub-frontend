  ...
  <view class="admin-page" :data-lang="currentLang">
    ...
    <view v-else class="list">
      <view v-for="item in list" :key="item.id" class="review-card">
        ...
        <view class="actions"><button v-if="item.status === 2 || item.status === 3" @tap="publish(item)">{{ t('guide.admin.publish') }}</button><button v-if="item.status === 1" @tap="takeDown(item)">{{ t('guide.admin.takeDown') }}</button><button class="danger" @tap="remove(item)">{{ t('guide.admin.delete') }}</button></view>
      </view>
    </view>
