    ...
    <view v-else-if="detail" class="detail-content">
      ...
      <view class="comment-card">
        ...
        <view v-for="comment in comments" :key="comment.id" class="comment-item">
          ...
          <view class="comment-main">
            <view>
              ...
              <text class="comment-time">{{ (comment.createTime || '').slice(0, 16) }}</text>
            </view>
        ...
        <view class="comment-input">
          <input v-model="commentText" :placeholder="t('guide.detail.commentPlaceholder')" maxlength="500" />
          <button @tap="submitComment">{{ t('guide.detail.commentSend') }}</button>
        </view>
      </view>
    </view>
